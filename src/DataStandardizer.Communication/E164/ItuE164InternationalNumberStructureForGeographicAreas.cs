...
namespace DataStandardizer.Communication.E164
{
    internal sealed class ItuE164InternationalNumberStructureForGeographicAreas : ItuE164InternationalNumberStructureBase, IItuE164InternationalNumberForGeographicAreas
    {
        ...
        private static string GetParsePattern(ItuE164InternationalNumberStyles numberStyles)
        {
            ...
            return string.Concat(@"^(?=(?:\D*\d){2,", MaximumDigitCount, @"}\D*$)", numberStyles.HasFlag(ItuE164InternationalNumberStyles.AllowLeadingWhite) ? $"[{PatternWhiteSpaceCharacterClass}]*": string.Empty,
                ...
                countryCodePattern, ")[", PatternSeparatorCharacterClass, "]*(?<", NumberPart.SubscriberNumber, @">\d(?:[", PatternSeparatorCharacterClass, @"]*\d){0,", MaximumDigitCount - 2, "})",
...
#endif
        {
            ...
            if (parseMatch.Success)
            {
                ...
                var concatenatedParts = Regex.Replace(countryCodePart + nationalSignificantNumberPart, @"\D", string.Empty);
