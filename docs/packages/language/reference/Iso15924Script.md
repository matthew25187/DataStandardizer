...
The ISO 15924 script code list. ISO 15924 defines a numeric code for each script,
so this is a conventional C# `enum`: each member is named after the standard's
four-letter script code (for example `Cyrl`), and the member's underlying value is
the standard's numeric code (for example `220`).
...
Per-member metadata — English and French names, alias, age, and date — is carried
by [Iso15924ScriptCodeAttribute](Iso15924ScriptCodeAttribute.md) and read through
[Iso15924Extensions](Iso15924Extensions.md).

## Fields

| Field | Numeric code | Script |
| --- | --- | --- |
| `Adlm` | 166 | Adlam |
| `Afak` | 439 | Afaka |
