...
namespace DataStandardizer.BCP47.Tests
{
    ...
    public class Bcp47LanguageTagTests : IClassFixture<Bcp47LanguageTagFixture>
    {
        ...
        private class CheckRegionSubtag_WithUnM49CodeSubtag_TestCaseFactory : IEnumerable<object[]>
        {
            ...
            {
                var globalCodes = Enum.GetValues<UnM49>().Select(code => code.GetGlobalCode()).Where(code => code is not null).Cast<ushort>();
                var regionCodes = Enum.GetValues<UnM49>().Select(code => code.GetRegionCode()).Where(code => code is not null).Cast<ushort>();
                var subRegionCodes = Enum.GetValues<UnM49>().Select(code => code.GetSubRegionCode()).Where(code => code is not null).Cast<ushort>();
                var intermediateRegionCode = Enum.GetValues<UnM49>().Select(code => code.GetIntermediateRegionCode()).Where(code => code is not null).Cast<ushort>();
        ...
        private class RegionSubtag_WhenSupranationalUnM49Code_TestCaseGenerator
        {
            public static IEnumerable<object[]> TestCases
            {
                get
                {
                    var globalM49Codes = Enum.GetValues<UnM49>().Select(code => code.GetGlobalCode()).Where(code => code is not null).Cast<ushort>().Distinct();
                    foreach (var globalM49Code in globalM49Codes)
                    {
                        yield return new object[] { $"en-{globalM49Code:000}", $"{globalM49Code:000}" };
                    }

                    var regionM49Codes = Enum.GetValues<UnM49>().Select(code => code.GetRegionCode()).Where(code => code is not null).Cast<ushort>().Distinct();
                    foreach (var regionM49Code in regionM49Codes)
                    {
                        yield return new object[] { $"en-{regionM49Code:000}", $"{regionM49Code:000}" };
                    }

                    var subRegionM49Codes = Enum.GetValues<UnM49>().Select(code => code.GetSubRegionCode()).Where(code => code is not null).Cast<ushort>().Distinct();
                    foreach (var subRegionM49Code in subRegionM49Codes)
                    {
                        yield return new object[] { $"en-{subRegionM49Code:000}", $"{subRegionM49Code:000}" };
                    }

                    var intermediateRegionM49Codes = Enum.GetValues<UnM49>().Select(code => code.GetIntermediateRegionCode()).Where(code => code is not null).Cast<ushort>().Distinct();
        ...
        private class RegionSubtag_WhenNationalUnM49Code_TestCaseFactory : IEnumerable<object[]>
        {
            ...
            {
                _testCases = Enum.GetValues<UnM49>().Cast<ushort>().Select(code => new object[] { $"en-{code:000}", code.ToString("000") }).AsEnumerable();
            }
